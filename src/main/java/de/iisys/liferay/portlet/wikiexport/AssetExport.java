...
//				content += JournalContentUtil.getContent(themeDisplay.getScopeGroupId(), ja.getArticleId(), null, themeDisplay.getLanguageId(), themeDisplay);
				...
				// TODO: JournalContentUtil does not exist anymore
//				contents.add(content+JournalContentUtil.getContent(themeDisplay.getScopeGroupId(), ja.getArticleId(), null, themeDisplay.getLanguageId(), themeDisplay));
			...
			// TODO: retrieve via user service or shindig
			// full user name
			String userFullName = request.getRemoteUser();
			// user screen name (ID in other systems)
			String userScreenName = request.getRemoteUser();
			
			try
			{
				User user = UserServiceUtil.getUserById(Long.parseLong(userFullName));
...
//			if(content!="") {
			if(contents.size()>0) {
				if(targetFormat.equals("pdf")) {
					docFile = WikiParser.createPDF(fileName, contents, syntax, authors, userFullName);
...
//					docFile = WikiParser.createPDF(fileName, content, authors, themeDisplay.getUser().getFullName());
				...
				if(targetFormat.equals("docx")) {
					docFile = WikiParser.createDOCX(fileName, contents, syntax, authors, userFullName, getPortletContext().getRealPath("/")+"templates/");
//					docFile = WikiParser.createDOCX(fileName, content, authors, themeDisplay.getUser().getFullName(), getPortletContext().getRealPath("/")+"templates/");
	...
	public void serveResource(ResourceRequest request, ResourceResponse response)
		...
		try
		{
			User user = UserServiceUtil.getUserById(Long.parseLong(userScreenName));
			userScreenName = user.getScreenName();
		}
		catch(Exception e)
		{
